...
let constructTree = function (r,id,d){
    ...
    for (let k of keys) {
        ...
        if(!isAttr && !isText && !isArray){
            // debugger;
            // console.log(k,"append it","isAttr",isAttr,"isText",isText,"isArray",isArray,"isString",this.isString);
...
class VirtualTreeComponent extends React.Component {
    ...
    render() {
        let result = {};
        if (this.props.data) {
            if(this.props.treeRef && this.props.treeRef.current){
                console.log("ref",this.props.treeRef)
            }else{
                console.log("ref not")
            }

            let rootKey = Object.keys(this.props.data)[0];
            constructTree.call(result, rootKey, rootKey, this.props.data);
